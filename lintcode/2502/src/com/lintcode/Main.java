...
public class Main {
    ...
    public static void main(String[] args) {
        try {
                ...
                appendThreads[i] = new Thread(() -> {
                    try {
                        for(int j = 0; j < appendOperationList.size(); j++) {
                            if(j % 15 == temp)
                                array.append(appendOperationList.get(j));
                        }
                    }catch (Exception e) {
                        e.printStackTrace();
                    }
            ...
            for(int i = 0;i < 15; i++) {
                if(appendThreads[i] != null)
                    appendThreads[i].join();
                if(i < 5 && getThreads[i] != null)
                    getThreads[i].join();
            }
            //test code
            int totalCounts = 0;
            ...
            if (totalCounts != array.size()){
                ...
                for(Integer i : solutionDataMap.keySet()){
                    if (solutionDataMap.get(i) != map.get(i)){
                        flag = false;
                        printStream.write(("Your number-" + String.valueOf(i) + " counts is " + String.valueOf(solutionDataMap.get(i)) + "\n").getBytes("Utf-8"));
